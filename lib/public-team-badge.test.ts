...
test("a jornada mostra oito jogos no desktop e navega sem scrollbar visível", async () => {
  ...
  assert.match(stripStyles, /grid-auto-columns:\s*calc\(\(100% - 42px\) \/ 8\)/);
  assert.match(stripStyles, /@media \(max-width:\s*1499px\)[\s\S]*?\/ 6\)/);
  assert.match(stripStyles, /@media \(max-width:\s*1023px\)[\s\S]*?\/ 4\)/);
  assert.match(stripStyles, /@media \(max-width:\s*639px\)[\s\S]*?\/ 2\)/);
  assert.match(stripStyles, /@media \(max-width:\s*399px\)[\s\S]*?grid-auto-columns:\s*100%/);
  ...
  assert.match(stripStyles, /min-height:\s*112px/);
