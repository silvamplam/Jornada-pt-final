...
test("a jornada mostra oito jogos no desktop e navega sem scrollbar visível", async () => {
  ...
  assert.match(stripStyles, /\.carouselViewport > \.row\s*\{[\s\S]*?display:\s*flex[\s\S]*?flex-wrap:\s*nowrap[\s\S]*?justify-content:\s*flex-start[\s\S]*?gap:\s*8px/);
  assert.doesNotMatch(stripStyles, /grid-auto-columns/);
  assert.match(stripStyles, /\.carousel\s*\{[\s\S]*?width:\s*1512px[\s\S]*?max-width:\s*calc\(100vw - 80px\)/);
  assert.match(stripStyles, /@media \(max-width:\s*1591px\)[\s\S]*?width:\s*1132px/);
  assert.match(stripStyles, /@media \(max-width:\s*1211px\)[\s\S]*?width:\s*752px/);
  assert.match(stripStyles, /@media \(max-width:\s*831px\)[\s\S]*?width:\s*372px/);
  assert.match(stripStyles, /@media \(max-width:\s*451px\)[\s\S]*?width:\s*182px/);
  ...
  assert.match(stripStyles, /flex:\s*0 0 182px;[\s\S]*?width:\s*182px;[\s\S]*?min-width:\s*182px;[\s\S]*?max-width:\s*182px;[\s\S]*?height:\s*132px/);
