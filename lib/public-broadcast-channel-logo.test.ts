...
test("PublicMatchStrip usa carrossel limpo e mantém o layout partilhado nos restantes contextos", async () => {
  ...
  assert.match(carouselSource, /viewport\.scrollBy\(\{[\s\S]*?behavior:\s*reducedMotion \? "auto" : "smooth"/);
  assert.match(carouselSource, /firstCard\.getBoundingClientRect\(\)\.width \+ gap/);
  assert.match(styleSource, /\.carousel\s*\{[\s\S]*?width:\s*1512px[\s\S]*?max-width:\s*calc\(100vw - 80px\)[\s\S]*?margin-inline:\s*auto/);
  ...
  assert.match(styleSource, /\.carouselViewport > \.row\s*\{[\s\S]*?display:\s*flex[\s\S]*?flex-wrap:\s*nowrap[\s\S]*?justify-content:\s*flex-start[\s\S]*?width:\s*max-content[\s\S]*?gap:\s*8px/);
  assert.doesNotMatch(styleSource, /grid-auto-columns/);
  assert.match(styleSource, /@media \(max-width:\s*1591px\)[\s\S]*?width:\s*1132px/);
  assert.match(styleSource, /@media \(max-width:\s*1211px\)[\s\S]*?width:\s*752px/);
  assert.match(styleSource, /@media \(max-width:\s*831px\)[\s\S]*?width:\s*372px/);
  assert.match(styleSource, /@media \(max-width:\s*451px\)[\s\S]*?width:\s*182px/);
  assert.match(styleSource, /\.carouselButton\s*\{[\s\S]*?color:\s*#44152f/);
  assert.match(styleSource, /\.carouselButtonBack\s*\{[\s\S]*?left:\s*-40px/);
  assert.match(styleSource, /\.carouselButtonForward\s*\{[\s\S]*?right:\s*-40px/);
  assert.match(styleSource, /\.panel\[data-visual-variant="clean"\] \.row > \.card \{[\s\S]*?flex:\s*0 0 182px[\s\S]*?grid-template-rows:\s*16px 25px 42px 23px[\s\S]*?width:\s*182px[\s\S]*?min-width:\s*182px[\s\S]*?max-width:\s*182px[\s\S]*?height:\s*132px[\s\S]*?background:\s*#ffffff/);
  assert.match(styleSource, /\.cleanSchedule[\s\S]*?align-items:\s*flex-start[\s\S]*?justify-content:\s*flex-start[\s\S]*?width:\s*100%[\s\S]*?text-align:\s*left/);
