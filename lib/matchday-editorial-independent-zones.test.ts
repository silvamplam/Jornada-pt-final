...
test("pagina publica controla as tres zonas de forma independente e permite simultaneidade", () => {
  ...
  assert.match(publicPageSource, /const highlightsAreActive = physicalSnapshot[\s\S]*?\? physicalHighlights\.length > 0[\s\S]*?: editorial[\s\S]*?\? editorial\.below_headline_mode !== "roundup"[\s\S]*?: referenceHighlightItems\.length > 0 \|\|[\s\S]*?continuityHighlights\.length > 0;/);
  assert.match(publicPageSource, /const roundupIsActive = physicalSnapshot[\s\S]*?\? physicalSnapshot\.video\.active[\s\S]*?: editorial[\s\S]*?\? complementaryMode === "roundup_video"[\s\S]*?: useReferenceRoundupItems;/);
  ...
  assert.match(publicPageSource, /const complementaryIsActive = physicalSnapshot[\s\S]*?\? physicalSnapshot\.video\.active && physicalVideoHighlight !== null[\s\S]*?: editorial[\s\S]*?\? editorial\.complementary_status === "published"[\s\S]*?: Boolean\(referenceComplement\);/);
