...
test("a barra partilhada permanece nos contextos validos e o separador Jogos mantem apenas a grelha grande", async () => {
  ...
  assert.match(cleanStyles, /\.carousel\s*\{[\s\S]*?max-width:\s*1512px[\s\S]*?margin-inline:\s*auto/);
  ...
  assert.match(cleanStyles, /grid-auto-columns:\s*182px/);
  assert.match(cleanStyles, /@media \(max-width:\s*1879px\)[\s\S]*?max-width:\s*1132px/);
  assert.match(cleanStyles, /@media \(max-width:\s*1399px\)[\s\S]*?max-width:\s*752px/);
  assert.match(cleanStyles, /@media \(max-width:\s*767px\)[\s\S]*?max-width:\s*372px/);
  assert.match(cleanStyles, /@media \(max-width:\s*399px\)[\s\S]*?max-width:\s*182px/);
  assert.match(cleanStyles, /\.row > \.card \{[\s\S]*?width:\s*182px;[\s\S]*?height:\s*132px;[\s\S]*?background:\s*#ffffff/);
