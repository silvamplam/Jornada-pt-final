...
test("a barra partilhada permanece nos contextos validos e o separador Jogos mantem apenas a grelha grande", async () => {
  ...
  assert.match(cleanStyles, /grid-auto-columns:\s*calc\(\(100% - 42px\) \/ 8\)/);
  assert.match(cleanStyles, /@media \(max-width:\s*1499px\)[\s\S]*?calc\(\(100% - 30px\) \/ 6\)/);
  assert.match(cleanStyles, /@media \(max-width:\s*1023px\)[\s\S]*?calc\(\(100% - 18px\) \/ 4\)/);
  assert.match(cleanStyles, /@media \(max-width:\s*639px\)[\s\S]*?calc\(\(100% - 6px\) \/ 2\)/);
  assert.match(cleanStyles, /@media \(max-width:\s*399px\)[\s\S]*?grid-auto-columns:\s*100%/);
  assert.match(cleanStyles, /\.row > \.card \{[\s\S]*?min-height:\s*112px;[\s\S]*?background:\s*#ffffff/);
  assert.match(cleanStyles, /\.cleanChannel \{[\s\S]*?grid-row:\s*4/);
