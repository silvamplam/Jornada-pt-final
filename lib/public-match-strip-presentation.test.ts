...
test("a barra partilhada permanece nos contextos validos e o separador Jogos mantem apenas a grelha grande", async () => {
  ...
  assert.match(stylesSource, /\.finishedScoreLine\s*\{[\s\S]*?--public-finished-score-inset:\s*clamp\(2px,\s*2\.5cqi,\s*4px\);[\s\S]*?grid-template-columns:\s*minmax\(0,\s*1fr\)\s*12px\s*minmax\(0,\s*1fr\);[\s\S]*?transform:\s*translateY\(-5px\)/);
