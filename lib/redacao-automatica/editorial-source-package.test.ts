...
test("a interface recolhe género, título e instruções e expõe as ações finais", () => {
  ...
  assert.doesNotMatch(submitEnhancer, /data-source-package-use-image/);
  assert.doesNotMatch(submitEnhancer, /Imagem escolhida/);
  assert.doesNotMatch(mainPage, /data-source-package-image-preference/);
  assert.doesNotMatch(mainPage, /data-source-package-image-summary/);
  assert.doesNotMatch(mainPage, /data-source-package-image-summary-list/);
  assert.doesNotMatch(mainPage, /Imagens dos artigos/);
  assert.match(mainPage, /className={styles\.simpleFeedImage}/);
  assert.match(mainPage, /<img src={article\.imageUrl}/);
  ...
  assert.doesNotMatch(submitEnhancer, /renderImageSummary/);
  assert.doesNotMatch(submitEnhancer, /Usar esta imagem/);
  ...
  assert.doesNotMatch(styles, /sourcePackageArticleImages/);
  assert.match(mainPage, /Cada Dossiê pode dar origem a um ou/);
  assert.match(mainPage, /Saídas editoriais do Dossiê/);
