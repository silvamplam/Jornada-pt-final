...
const publicMatchdayStyles = `
  ...
  .public-season-context-card .public-matchday-leg-nav a:hover {
    background: #edf1f6;
    color: #17283b;
  }

  .public-season-context-card .public-season-select-wrap:focus-within,
  .public-season-context-card .public-matchday-leg-nav a:focus-visible,
  .public-matchday-nav-compact a:focus-visible {
    outline: 2px solid #59718a;
    outline-offset: 2px;
  }
...
export default async function PublicMatchdayPage({ params, searchParams }: PublicMatchdayPageProps) {
  ...
  const thematicSnapshot =
    thematicRead?.kind === "thematic"
    && thematicRead.competitionSlug
    ...
    thematicSnapshot
      ...
      : editorial?.latest_zone_placement === "hidden"
        ...
        : editorial?.latest_zone_placement === "four_news"
          ? "four_news"
          : "top";
  const configuredLatestZoneTitle = thematicSnapshot
    ...
    <main className="public-matchday-shell">
      ...
      {thematicPublicUnavailable ? (
        <section
          ...
          aria-label="Composição temática indisponível"
          data-public-thematic-state="unavailable"
        >
          <p>
            A composição editorial temática desta jornada está temporariamente indisponível.
          </p>
        </section>
      ) : useHierarchicalReferenceComposition ? (
