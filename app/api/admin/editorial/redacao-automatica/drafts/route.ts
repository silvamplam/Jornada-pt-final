...
import { createNewsroomEditorialDraft } from "@/lib/redacao-automatica/editorial-draft-service";
...
export async function POST(request: Request) {
  ...
  if (!newsroomArticleId) {
    return redirectTo(returnTo, { draft_error: "input_invalid" });
  }

  const result = await createNewsroomEditorialDraft(newsroomArticleId);
  if (!result.ok) {
    return redirectTo(returnTo, { draft_error: result.error.code });
  }

  return redirectTo("/admin/editorial/artigos", {
    articleId: result.value.editorialArticle.id,
    newsroom_draft: result.value.action,
  });
}
