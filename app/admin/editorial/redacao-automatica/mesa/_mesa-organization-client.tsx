...
export function MesaLooseSourcesPanel({
  ...
  const items = tab === "new" ? newItems : tab === "published" ? publishedItems : archiveItems;
  const empty = tab === "new"
  ...
  return <section className={styles.sourcePanel} data-lifecycle={tab}>
    <header className={styles.panelHeader}><nav aria-label="Fontes">
      ...
      <Link href={archiveHref} aria-current={tab === "archive" ? "page" : undefined}>ARQUIVO ({archiveCount})</Link>
    </nav></header>
    <MesaSourceWindow
      key={tab}
