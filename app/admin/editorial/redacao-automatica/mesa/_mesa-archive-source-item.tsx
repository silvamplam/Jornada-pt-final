...
import type { MesaArchiveSourceItem } from "@/lib/redacao-automatica/newsroom-mesa-archive-read-model";
...
export function MesaArchiveSourceItemView({ item }: Readonly<{
  ...
  async function reopen() {
    if (!item.newsroomSnapshotId || !item.cycleEligible || busy) return;
    setBusy(true);
    setMessage("");
    try {
      ...
      if (!response.ok || !result?.ok) {
        throw new Error(result?.message || "Não foi possível repor esta fonte na Mesa.");
      }
      router.refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Não foi possível repor esta fonte na Mesa.");
    } finally {
      setBusy(false);
    }
  }
    ...
    <li className={styles.sourceRow} data-lifecycle="archive">
      ...
      <article className={styles.sourceBody}>
        <div className={styles.sourceMeta}>
          <span className={styles.lifecycleBadge}>ARQUIVO</span>
        ...
        <div className={styles.sourceTools}>
          ...
          {item.cycleEligible ? (
            <button
              type="button"
              disabled={busy || !item.newsroomSnapshotId}
              onClick={() => void reopen()}
            >
