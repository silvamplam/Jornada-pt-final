...
alter table public.matchday_reference_compositions
  add column if not exists hierarchical_faixa_title text;

alter table public.matchday_reference_compositions
  drop constraint if exists matchday_reference_compositions_hierarchical_faixa_title_check;

alter table public.matchday_reference_compositions
  add constraint matchday_reference_compositions_hierarchical_faixa_title_check
  check (
    hierarchical_faixa_title is null
    or (
      hierarchical_faixa_title = pg_catalog.btrim(hierarchical_faixa_title)
      and pg_catalog.char_length(hierarchical_faixa_title) between 1 and 120
    )
  );

comment on column public.matchday_reference_compositions.hierarchical_faixa_title is
  'Título público opcional da Faixa da composição histórica. Null significa ausência de heading público.';
...
begin
  ...
  if v_has_faixa_title then
    if pg_catalog.jsonb_typeof(p_settings -> 'faixaTitle') <> 'string' then
      raise exception 'historical_faixa_title_invalid';
    end if;

    v_faixa_title := pg_catalog.btrim(p_settings ->> 'faixaTitle');

    if pg_catalog.char_length(v_faixa_title) > 120 then
      raise exception 'historical_faixa_title_invalid';
    end if;
  end if;
  ...
  v_total := public.apply_historical_composition_workspace_plan_v2(
    ...
    case
      ...
      else p_settings - 'videoPosition' - 'faixaTitle'
    end,
    p_dynamic_zones
  );
  ...
  if v_has_video_position or v_has_faixa_title then
    update public.matchday_reference_compositions as composition
    set
      hierarchical_video_position = case
        when v_has_video_position then v_video_position
        else composition.hierarchical_video_position
      end,
      hierarchical_faixa_title = case
        when v_has_faixa_title then nullif(v_faixa_title, '')
        else composition.hierarchical_faixa_title
      end,
      updated_at = now()
    ...
    if not found then
      raise exception 'historical_workspace_settings_not_editable';
    end if;
  end if;

  return coalesce(v_total, 0)
    + case when v_has_video_position then 1 else 0 end
    + case when v_has_faixa_title then 1 else 0 end;
end
$$;
...
comment on function public.apply_historical_composition_workspace_plan_v3(
  ...
  'Aplica atomicamente a montagem histórica, as zonas dinâmicas, a posição do Vídeo + Destaque e o título público opcional da Faixa.';
...
begin
  ...
  values (
    ...
    v_source.hierarchical_faixa_title,
