...
begin
  ...
  if exists (
    ...
    where pg_catalog.jsonb_typeof(zone.value) <> 'object'
       or nullif(pg_catalog.btrim(zone.value ->> 'publicTitle'), '') is null
...
create or replace function public.apply_historical_composition_workspace_plan_v2(
  p_matchday_id uuid,
  p_composition_id uuid,
  p_operations jsonb,
  p_settings jsonb,
...
begin
  v_base :=
    public.apply_historical_composition_workspace_plan(
      p_matchday_id,
      p_composition_id,
      p_operations,
      p_settings
  ...
  v_dynamic :=
    public.replace_historical_composition_dynamic_zones(
      p_matchday_id,
      p_composition_id,
      p_dynamic_zones
    );
  ...
  return
    coalesce(v_base, 0)
    + coalesce(v_dynamic, 0);
end
$$;

revoke all
on function public.apply_historical_composition_workspace_plan_v2(
  uuid,
  uuid,
  jsonb,
  jsonb,
...
is
  'Substitui atomicamente as zonas editoriais variáveis de uma composição histórica usando snapshots de artigos canónicos publicados.';

comment on function
  public.apply_historical_composition_workspace_plan_v2(
    uuid,
    uuid,
    jsonb,
    jsonb,
...
is
  'Aplica a montagem fixa e as zonas editoriais variáveis dentro da mesma transação PostgreSQL.';
