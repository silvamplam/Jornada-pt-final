...
begin
  ...
  if exists (
    ...
    where
      item ? 'preserve'
      and jsonb_typeof(item -> 'preserve') is distinct from 'boolean'
  ) then
    raise exception 'agenda-tv-v2-invalid-preserve';
  end if;
  ...
  if exists (
    ...
    where
      ...
      or (
        not coalesce((item ->> 'preserve')::boolean, false)
        and (
          nullif(item ->> 'scheduled_date', '') is null
  ...
  if exists (
    ...
    where
      not coalesce((item ->> 'preserve')::boolean, false)
      and nullif(item ->> 'broadcast_channel_id', '') is not null
      and not exists (
        select 1
...
is
  'Aplica data/hora de uma jornada de forma atómica; preserve=true valida o estado esperado sem escrever o jogo; canal TV só muda quando existe confirmação exata no catálogo.';
