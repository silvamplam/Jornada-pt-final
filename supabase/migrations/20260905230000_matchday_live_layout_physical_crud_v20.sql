...
-- LOTE 3 / V20
--
-- Forward-only physical topology CRUD. The final-state payload remains the
-- authority; legacy tables are a best-effort derived compatibility subset.
-- The administrative route intentionally remains on v14 until its preview
-- state can construct create/delete operations and displacement events.
-- ============================================================

-- ============================================================
-- 1. ONE SQL LAYOUT DEFINITION
-- ============================================================
...
begin
  if p_matchday_id is null
    or p_profile_key is distinct from 'liga_portugal_v1'
  then
    raise exception 'matchday-live-layout-physical-v20-profile-unsupported';
  end if;

  -- The table PK, key CHECK, unique zone identity and deferred contextual FK
...
begin
  ...
  -- Special placements and only mapped physical zones are materialized. The
  -- v14 projector already deletes stale zone_items and omits unmapped zones.
  perform
  ...
  into v_zone_order, v_zone_layouts, v_zone_titles
  ...
  begin
    -- The old five-key topology is updated only when it is completely
    -- representable. Extra physical zones are deliberately omitted.
    if v_projection_count = 5
      and pg_catalog.cardinality(v_zone_order) = 5
      and pg_catalog.cardinality(v_block_order) in (6, 7)
      and v_block_order @> array['latest']::text[]
    then
    ...
    -- Presentation settings remain useful to historical consumers even when
    -- the zone topology itself cannot be represented by five thematic keys.
...
begin
  ...
  if v_video_module_active then
    ...
    if v_roundup_item_id is null then
      raise exception 'matchday-live-layout-physical-v20-video-required';
    end if;
  ...
  from jornada_private.apply_matchday_live_layout_physical_workspace_v20_core(
    p_matchday_id,
    p_profile_key,
    p_expected_physical_state_token,
    p_zones,
...
begin
  if pg_catalog.has_function_privilege(
       'anon',
       'public.apply_matchday_live_layout_physical_v20(uuid,text,text,jsonb,jsonb,jsonb,integer,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb)'::pg_catalog.regprocedure,
       'EXECUTE'
     )
    ...
    or pg_catalog.has_function_privilege(
       'service_role',
       'jornada_private.apply_matchday_live_layout_physical_workspace_v20_core(uuid,text,text,jsonb,jsonb,jsonb,integer,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb)'::pg_catalog.regprocedure,
       'EXECUTE'
     )
  then
