...
begin
  ...
  if v_source_type = 'editorial_article' then
    v_public_url := case
      when nullif(btrim(coalesce(old.slug, '')), '') is null then null
      else '/noticias/' || old.slug
    end;

    if v_public_url is not null then
      delete from public.matchday_latest_news latest_row
      where regexp_replace(
              split_part(split_part(coalesce(latest_row.link_url, ''), '?', 1), '#', 1),
              '/$',
              ''
