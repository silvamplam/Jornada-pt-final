...
-- LOTE 7C3
--
-- As quatro ao lado das Últimas possuem quatro posições físicas,
-- mas a ocupação editorial válida é 0..4.
--
-- O cutover já retirou a implementação V9 da API pública:
--
-- public V9
--   -> jornada_private V9 pre_cutover
--      -> jornada_private V9 pre_bridge
--
-- Esta migration altera exclusivamente a implementação privada
-- pre_bridge. O envelope público, o lock de cutover e o adaptador
-- autoritativo permanecem intactos.
...
begin
  select pg_catalog.pg_get_functiondef(
    'public.apply_matchday_editorial_profile_workspace_v9(uuid,text,bigint,text,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb)'::pg_catalog.regprocedure
  )
  into v_public_v9;

  select pg_catalog.pg_get_functiondef(
    'jornada_private.apply_matchday_editorial_profile_workspace_v9_pre_cutover(uuid,text,bigint,text,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb)'::pg_catalog.regprocedure
  )
  into v_pre_cutover;
  ...
  then
    raise exception
      'matchday-selection-optional-v9-pre-cutover-wrapper-missing';
  end if;
end;
...
begin
  ...
  if p_selection_bank_item_ids is null
    ...
    or exists (
      ...
      where pg_catalog.jsonb_typeof(selection_row.value)
          not in ('string', 'null')
        or (
          pg_catalog.jsonb_typeof(selection_row.value) = 'string'
          and nullif(
...
is
  'Private pre-bridge completeness guard. The four-news surface has four positional slots with valid occupancy from zero to four; non-null bank items remain unique.';
...
begin
  select pg_catalog.pg_get_functiondef(
    'jornada_private.apply_matchday_editorial_profile_workspace_v9_pre_bridge(uuid,text,bigint,text,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb)'::pg_catalog.regprocedure
  )
  into v_private_v9;
  ...
  then
    raise exception
      'matchday-selection-optional-v9-shape-guard-missing';
  end if;
  ...
  then
    raise exception
      'matchday-selection-optional-public-wrapper-changed';
  end if;
end;
$postcondition$;
