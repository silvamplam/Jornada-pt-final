...
-- SQL 2/4 — APLICAÇÃO MANUAL. Substitui apenas a política da RPC existente.
...
begin
  ...
  if found then
    ...
    if v_article.external_id is not null
       and p_external_id is not null
       and v_article.external_id is distinct from p_external_id then
      raise exception 'persistence_conflict'
        using detail = 'article', errcode = 'P0001';
    end if;
    ...
    v_effective_original_url := case
      when v_apply_mutable_metadata then p_original_url
      else v_article.original_url
    end;
    v_effective_published_at := case
      when v_article.published_at is null and p_published_at is not null
        then p_published_at
      when v_apply_mutable_metadata
        then coalesce(p_published_at, v_article.published_at)
      else v_article.published_at
    end;
    v_mutable_metadata_changes :=
      v_article.original_url is distinct from v_effective_original_url
      or v_article.published_at is distinct from v_effective_published_at
      or (
    ...
    if v_external_id_changes or v_mutable_metadata_changes then
      ...
      set
        ...
        published_at = v_effective_published_at,
        modified_at = case
  ...
  if found then
    ...
    if v_snapshot.body is distinct from p_body then
      raise exception 'persistence_conflict'
        using detail = 'snapshot', errcode = 'P0001';
    end if;
...
comment on function public.newsroom_persist_article_snapshot(
  ...
  'Atomically persists one canonical newsroom article and one immutable content snapshot; preserves known publication dates and reuses technically equivalent snapshots; service_role only.';
