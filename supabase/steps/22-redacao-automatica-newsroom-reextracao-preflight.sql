...
checks as (
  select
    ...
    coalesce((
      ...
      from exact_rpc
    ), false) as extracted_at_compatibility_present,
    coalesce((
...
select jsonb_build_object(
  ...
  function_exists
    ...
    and extracted_at_compatibility_present
    and snapshot_update_absent
    and snapshot_delete_absent,
  'current_policy',
  case
    when body_compatibility_present
      and source_metadata_compatibility_present
      and extracted_at_compatibility_present
    then 'body_source_metadata_and_extracted_at'
    else 'unexpected_or_already_changed'
  end,
  ...
  'source_metadata_compatibility_present',
    source_metadata_compatibility_present,
  'extracted_at_compatibility_present',
    extracted_at_compatibility_present,
  'snapshot_mutation_absent',
    snapshot_update_absent and snapshot_delete_absent,
  'writes_performed', false,
  'next_step',
    'run 23-redacao-automatica-newsroom-reextracao-apply.sql manually'
