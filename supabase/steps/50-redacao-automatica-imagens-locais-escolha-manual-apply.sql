...
begin
  if to_regprocedure(
    'public.newsroom_apply_editorial_dossier_article_plan_generation(uuid,uuid,uuid,timestamptz,text,text,text,text,text,text,text,jsonb,integer,integer,integer)'
  ) is null then
...
create or replace function public.newsroom_apply_generated_article(
  p_dossier_id uuid,
...
begin
  ...
  if p_image_url is not null and btrim(p_image_url) <> '' then
    raise exception 'editorial_generation_image_must_be_selected_manually'
      using errcode = '23514';
  end if;
...
alter function public.newsroom_apply_generated_article(
  uuid, uuid, uuid, timestamptz, text, text, text, text, text, text, text, text, text, jsonb, integer, integer, integer
) owner to postgres;

revoke all on function public.newsroom_apply_generated_article(
  uuid, uuid, uuid, timestamptz, text, text, text, text, text, text, text, text, text, jsonb, integer, integer, integer
) from public, anon, authenticated;

grant execute on function public.newsroom_apply_generated_article(
  uuid, uuid, uuid, timestamptz, text, text, text, text, text, text, text, text, text, jsonb, integer, integer, integer
) to service_role;

comment on function public.newsroom_apply_generated_article(
  uuid, uuid, uuid, timestamptz, text, text, text, text, text, text, text, text, text, jsonb, integer, integer, integer
) is 'Applies one immutable editorial generation, fills title, post-title and body, and leaves the article image empty for manual selection.';

drop function if exists public.newsroom_apply_complete_editorial_dossier_article_plan_generati(
  uuid, uuid, uuid, timestamptz, text, text, text, text, text, text, text, text, text, jsonb, integer, integer, integer
);
