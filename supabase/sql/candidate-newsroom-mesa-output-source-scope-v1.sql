-- Candidate only: conservative per-output focus for Mesa production intents.
-- The parent selection/context remains complete; focus never authorizes or removes sources.
begin;
...
begin
  ...
  then raise exception 'mesa-output-focus-v1-preflight-missing'; end if;
end;
$preflight$;

create function public.newsroom_mesa_intent_output_focus_source_ids_v1(
  p_context jsonb,p_request jsonb,p_output jsonb,p_outputs jsonb
)
returns uuid[]
language plpgsql immutable set search_path=''
as $function$
declare
  v_all uuid[]; v_explicit uuid[]; v_theme_sources uuid[]; v_selected_themes uuid[];
  v_evidence_sources uuid[]; v_evidence_themes uuid[]; v_selected uuid[];
  v_reviewed uuid[]:='{}'::uuid[]; v_remaining uuid[]; v_existing jsonb; v_existing_focus uuid[];
  v_touches_theme boolean:=false; v_has_existing boolean:=false;
begin
  ...
  where source->>'newsroomArticleId' ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$';

  if cardinality(v_all)<1 or p_context->>'kind'<>'selection'
  ...
  into v_explicit from jsonb_array_elements(coalesce(p_request->'selection'->'sourceIds','[]'::jsonb))
  where value#>>'{}' ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$';

  select coalesce(array_agg(distinct lower(value#>>'{}')::uuid order by lower(value#>>'{}')::uuid),'{}'::uuid[])
  into v_selected_themes from jsonb_array_elements(coalesce(p_request->'selection'->'themeIds','[]'::jsonb))
  where value#>>'{}' ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$';

  select coalesce(array_agg(id order by id),'{}'::uuid[]) into v_theme_sources
  from unnest(v_all) source(id) where not(source.id=any(v_explicit));

  if p_output->>'kind'='existing' then
    if jsonb_typeof(p_output->'target'->'evidence') is distinct from 'object' then return null; end if;
    select coalesce(array_agg(distinct id order by id),'{}'::uuid[]) into v_evidence_sources
    from (
