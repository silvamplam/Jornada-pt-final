...
-- HOTFIX ON CONFLICT 1
-- Substitui a RPC já aplicada para eliminar a ambiguidade entre a coluna
-- article_plan_id e a variável de saída homónima da função.
--
-- A alteração funcional é exclusivamente:
-- ON CONFLICT ON CONSTRAINT newsroom_editorial_dossier_article_plan_sources_plan_source_key
--
-- Não cria tabelas nem altera dados existentes.
...
revoke all on function public.newsroom_save_editorial_dossier_article_plan(
  uuid,
  uuid,
  text,
  text,
...
grant execute on function public.newsroom_save_editorial_dossier_article_plan(
  uuid,
  uuid,
  text,
  text,
...
comment on function public.newsroom_save_editorial_dossier_article_plan(
  uuid,
  uuid,
  text,
  text,
  ...
  'Atomically creates or updates one dossier article plan and its ordered frozen-source assignments. Cancelled plans preserve their assignments.';
